...
def declare_logic():
    ...
    # Counts the number of unpaid sales for a customer
    Rule.count(derive=Customer.unpaid_sales_count, as_count_of=Sale, where=lambda row: not row.paid)

    # Sum of car stock in inventory should not exceed dealer's limits.
    Rule.constraint(validate=Dealer, as_condition=lambda row: row.inventory_limit >= sum(i.stock_amount for i in row.cars), error_msg='Dealer inventory limit exceeded')
